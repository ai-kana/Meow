...
public class FishingManager
{
    ...
    private static void SendFishingReward(KronstadtPlayer player)
    {
        FishingSkill skill = player.FishingSkill;
        uint level = skill.Level;

        IEnumerable<LootItem> items = KronstadtHost.Configuration.GetSection("Fishing").Get<IEnumerable<LootItem>>() ?? throw new("Fishing section null");
    ...
    {
        if (!KronstadtPlayerManager.TryGetPlayer(__instance.player, out KronstadtPlayer player))
        {
            return false;
        }
